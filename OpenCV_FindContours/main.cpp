...
int main (){
    ...
    Mat image = cv::imread("C:/Users/Sergey/Documents/_Qt_projects/projects/opencv_test/IMG.jpg");
    if(image.empty()){
        std::cout << "not image";
        return -1;
    }
    ...
    for(size_t i=0; i<contours.size(); ++i){
        ...
        if (contours[i].size()){
            ...
            if (approxCurve.size()){
                ...
                {
                    putText(image_copy, "triangle", center, FONT_HERSHEY_PLAIN, 2, {255,255,255});
                }
                else if(approxCurve.size() == 4)
                {
                    if (rect.width/100 == rect.height/100)
                    {
                        putText(image_copy, "square", center, FONT_HERSHEY_PLAIN, 2, {255,255,255});
                    }
                    else
                    {
                        putText(image_copy, "rectangle", center, FONT_HERSHEY_PLAIN, 2, {255,255,255});
                    }
                }
                else if(approxCurve.size() > 8)
                {
                    putText(image_copy, "circle", center, FONT_HERSHEY_PLAIN, 2, {255,255,255});
                }
            }
        }
    }
