...
void MainWindow::OpenVideo(){
    ...
    QString file_name = QFileDialog::getOpenFileName(this, tr("Open Video"), "./", tr("Video Files (*.mp4 *.avi *wmv)"));
...
void MainWindow::UpdateData(){
    if (orig_video.read(orig_img)){
        ...
        if (ui->VideoData->isChecked()){
            ...
            cv::putText(orig_img, s, {10,50}, cv::FONT_HERSHEY_PLAIN, 4, {255,255,255}, 3);
            cv::putText(orig_img, "CURRENT FRAME", {220,50}, cv::FONT_HERSHEY_PLAIN, 4, {255,255,255}, 3);
            cv::putText(cpyorig_img, s, {10,50}, cv::FONT_HERSHEY_PLAIN, 4, {255,255,255}, 3);
            cv::putText(cpyorig_img, "CURRENT FRAME", {220,50}, cv::FONT_HERSHEY_PLAIN, 4, {255,255,255}, 3);
            int end_frame = orig_video.get(cv::CAP_PROP_FRAME_COUNT);
            s = std::to_string(end_frame);
            cv::putText(orig_img, s, {10,100}, cv::FONT_HERSHEY_PLAIN, 4, {255,255,255}, 3);
            cv::putText(orig_img, "END FRAME", {220,100}, cv::FONT_HERSHEY_PLAIN, 4, {255,255,255}, 3);
            cv::putText(cpyorig_img, s, {10,100}, cv::FONT_HERSHEY_PLAIN, 4, {255,255,255}, 3);
            cv::putText(cpyorig_img, "END FRAME", {220,100}, cv::FONT_HERSHEY_PLAIN, 4, {255,255,255}, 3);
        }
        ...
        mat_transform_trapeze = cv::getPerspectiveTransform(trapeze, transform_trapeze);
        cv::warpPerspective(cpyorig_img, mat_trapeze, mat_transform_trapeze, cv::Size(480, 272));
        ...
        debug_image = QImage(HSV_img.data, HSV_img.cols, HSV_img.rows, QImage::Format_Grayscale8);
        ...
        if (ui->TrapezeOn->isChecked()){
            ...
            image = QImage(cpyorig_img.data, cpyorig_img.cols, cpyorig_img.rows, QImage::Format_BGR888);
            pixel = QPixmap::fromImage(image);
            ui->OriginVideo->setPixmap(pixel);

            cv::cvtColor(mat_trapeze, mat_HSV_trapeze, cv::COLOR_BGR2HSV);
            cv::inRange(mat_HSV_trapeze, cv::Scalar(0,0,200), cv::Scalar(70,255,255), mat_HSV_trapeze);
            ...
            for (auto it:left_points){
                cv::rectangle(mat_trapeze, cv::Point(it.x, it.y), cv::Point(it.x+20, it.y+20), cv::Scalar{0,255,0}, 1);
            }
            for (auto it:right_points){
                cv::rectangle(mat_trapeze, cv::Point(it.x, it.y), cv::Point(it.x+20, it.y+20), cv::Scalar{0,0,255}, 1);
            }
            ...
            cv::line(mat_trapeze, left_line[0], left_line[1], cv::Scalar{0,255,0}, 3);
            cv::line(mat_trapeze, right_line[0], right_line[1], cv::Scalar{0,0,255}, 3);

            std::vector<cv::Point> tmp = {left_line[0], right_line[0], right_line[1], left_line[1]};
            const cv::Point* elementPoints[1] = { &tmp[0] };
            int numberOfPoints = (int)tmp.size();
            cv::fillPoly (mat_trapeze, elementPoints, &numberOfPoints, 1, cv::Scalar (255, 255, 128), 8);

            cv::resize(mat_trapeze, mat_trapeze, cv::Size(480,272));
            image = QImage(mat_trapeze.data, mat_trapeze.cols, mat_trapeze.rows, QImage::Format_BGR888);
            pixel = QPixmap::fromImage(image);
            ui->ProcessOrigVideo->setPixmap(pixel);

            debug_image = QImage(mat_HSV_trapeze.data, mat_HSV_trapeze.cols, mat_HSV_trapeze.rows, QImage::Format_Grayscale8);
            debug_pixel = QPixmap::fromImage(debug_image);
            ui->ProcessDebugVideo->setPixmap(debug_pixel);
        } else {
            cv::resize(orig_img, orig_img, cv::Size(480,272));
            image = QImage(orig_img.data, orig_img.cols, orig_img.rows, QImage::Format_BGR888);
            pixel = QPixmap::fromImage(image);
            ui->OriginVideo->setPixmap(pixel);
        }
    }
