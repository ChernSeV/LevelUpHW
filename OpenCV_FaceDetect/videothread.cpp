...
videothread::videothread(int device, cv::Mat *frame, bool *face, bool *eyes, bool *lips,
                         QObject *parent, bool b) :
...
void videothread::run(){
    ...
    while(Stop == false){
        ...
        if (*FaceDetect){
            ...
            if (m_face.size () != 0) {
                for (auto it : m_face) {
                    cv::rectangle (mat_capture, it, cv::Scalar{ 0, 255, 0 }, 3, cv::LINE_8);
                }
            }
        }
        if (*EyesDetect){
            ...
            if (m_eyes.size () != 0) {
                for (auto it : m_eyes) {
                    cv::rectangle (mat_capture, it, cv::Scalar{ 0, 255, 255 }, 3, cv::LINE_8);
                }
            }
        }
        if (*LipsDetect){
            ...
            if (m_lips.size () != 0) {
                for (auto it : m_lips) {
                    cv::rectangle (mat_capture, it, cv::Scalar{ 255, 255, 0 }, 3, cv::LINE_8);
                }
            }
        }
